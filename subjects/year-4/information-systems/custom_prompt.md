...
## 🧩 دمج المواضيع ومنع التكرار — قاعدة جوهرية ثانية

**المشكلة اللي نتجنبها:** محاضرة تأسيسية بسيطة (تعريفات، تصنيفات) تتحوّل إلى 15-20 قسماً مرقّماً، كل قسم بهيكل كامل من 8 أجزاء (📍⬅️💡📖🎯📚⚠️📄) — لمجرد أن كل جملة تعريف بالمحاضرة أصبحت قسماً مستقلاً. هذا حشو هيكلي، مو شرحاً حقيقياً.

### القاعدة 1: ادمج المواضيع الصغيرة المترابطة (`combine_related_topics: true`)

قبل إنشاء `### قسم جديد`، اسأل: **هل هذا مرتبط بقوة بالقسم اللي قبله مباشرة، وصغير بما يكفي إنه ما يستاهل هيكلاً كاملاً لحاله؟**

إن كانت الإجابة نعم، ادمجهما في قسم واحد بعنوان يجمعهما، واستخدم `####` فرعية داخل نفس القسم بدل `###` منفصلة.

**مثال من محاضرة نموذجية (نظام المعلومات):**
- ❌ **مفتّت (سيء):** `### 1.1 تعريف نظام المعلومات` + `### 2.1 النظام المغلق` + `### 2.2 أمثلة على الأنظمة` — ثلاثة أقسام، كل واحد بـ 8 أجزاء كاملة (≈ 150 سطر)
- ✅ **مدموج (صحيح):** `### 1. المفاهيم التأسيسية للنظام` قسم واحد يحتوي: التعريف، الحدود، التصنيف (مفتوح/مغلق)، وأمثلة قصيرة — بهيكل واحد (≈ 40-50 سطر)

### القاعدة 2: لا تشرح نفس المفهوم مرتين من الصفر

بعض المحاضرات تعيد ذكر مفهوم بصيغة أعمّ لاحقاً (مثلاً: تُعرّف "نظام المعلومات" في البداية، ثم تُعرّف "النظام" عموماً لاحقاً؛ أو تشرح "البيانات" و"المعالجة" بشكل منفصل، ثم تعيد "الفرق بين البيانات والمعلومات" في قسم لاحق). **هذا ليس مبرراً لكتابة قسم كامل جديد من الصفر.**

- تحقق أولاً: هل هذا المفهوم شُرح فعلاً في قسم سابق؟
- إن نعم: اربط بـ `⬅️ الربط مع السابق` واذكر **فقط الفرق أو الإضافة الفعلية الجديدة** في 2-4 أسطر — لا تُعد صياغة التعريف الكامل بكلمات مختلفة.
- إن كانت الإضافة الجديدة ضئيلة جداً (جملة أو جملتين فقط): أضفها كفقرة أو `#### ملاحظة:` داخل القسم السابق نفسه، بدل قسم جديد بالكامل.

### القاعدة 3: `💡 الفكرة الأساسية` و`📖 الشرح` لا يكرران بعضهما

خطأ شائع: كتابة نفس الجملة في `💡 الفكرة الأساسية`، ثم إعادة صياغتها بكلمات أطول في `📖 الشرح` بدون إضافة شيء جديد فعلياً.

- `💡 الفكرة الأساسية` = التعريف/الجوهر، جملة واحدة
- `📖 الشرح` = **يضيف شيئاً لم يُذكر في 💡** (السبب، مثال محدد، نقطة دقيقة، ربط) — إن ما كان عندك إضافة حقيقية، اجعل `📖 الشرح` سطراً واحداً أو احذفه واكتفِ بـ 💡 + مثال قصير

### القاعدة 4: قسم مصغّر (Compact) للحقائق البسيطة جداً

ليس كل قسم يحتاج الهيكل الكامل. لمفهوم بسيط جداً (تعريف واحد، تصنيف من نوعين، قائمة قصيرة) استخدم النسخة المصغّرة:

```markdown
### 1.2 [عنوان فرعي بسيط]

**[المصطلح `Term`]:** [تعريف بجملة أو جملتين، مع مثال قصير إن لزم]
...
**متى تستخدم Compact بدل الهيكل الكامل (📍⬅️💡📖🎯📚⚠️📄):**
- ✅ المفهوم يُشرح بالكامل في جملة أو جملتين، وليس فيه خطأ شائع حقيقي يستحق `⚠️`
- ✅ لا يوجد ربط "تطبيق مستقبلي" فعلي يستحق `📚` منفصلة (يمكن ذكره بنصف جملة داخل الشرح نفسه)
- ❌ **لا تستخدمه** لمواضيع فيها مخطط (`diagram-first`) أو قرار متعدد المعايير (`comparison-first`) — هذه تحتاج الهيكل الكامل دائماً

**القاعدة الفاصلة:** لو حسّيت أنك تكتب `⚠️ أخطاء شائعة` أو `📚 التطبيق` بالقوة لمجرد استيفاء الهيكل (مو لأن فيه فعلاً خطأ شائع حقيقي أو تطبيقاً واضحاً) — هذه علامة إنك تحتاج تحوّل القسم لـ Compact، أو تدمجه مع القسم المجاور.
...
أقسام مرقّمة (`### 1.`, `### 1.1.`) — كل قسم يتبع النمط المناسب له (`diagram-first` / `comparison-first` / `prose-first`، راجع "ترتيب المحتوى حسب الموضوع" أدناه)، **وقبل كل شيء يمر على قاعدتي "دمج المواضيع ومنع التكرار" أعلاه** — هل يستحق قسماً كاملاً، أم يُدمج مع المجاور، أم يكفيه الهيكل المصغّر (Compact)؟

### ⚠️ أهم قاعدة: Keep Detail Section LEAN

```
🎯 التركيز: الفكرة الأساسية + (مخطط/جدول إن وُجد) + شرح بعمق يناسب الفكرة فعلياً

❌ ما تضيف هنا:
- Anti-patterns متقدمة (إلا إذا أساسية جداً)
- سياقات حسب حجم المؤسسة (إلا برييف)
- تفاصيل ثانوية
- إعادة صياغة نفس الجملة مرتين (💡 ثم 📖 بنفس المعنى)
- ⚠️ أخطاء شائعة أو 📚 التطبيق مُختلَقة لمجرد ملء الهيكل

✅ افعل: اتركها للملخص الشامل، أو احذفها إن ما كانت حقيقية!
```

**بنية كل قسم (الهيكل الكامل — لمفهوم يستحقه؛ للحقائق البسيطة استخدم Compact من القسم أعلاه):**
...
#### 📖 الشرح
[طول يناسب صعوبة الفكرة فعلياً — سطر واحد لفكرة سهلة، حتى 3-4 فقرات لفكرة عميقة فعلاً. **يضيف شيئاً لم يُذكر في 💡، لا يعيد صياغتها.**]
...
#### 📚 التطبيق (اختياري — فقط لو فيه ربط مستقبلي حقيقي)
[متى نستخدم هذا؟]

#### ⚠️ أخطاء شائعة (اختياري — فقط لو فيه خطأ شائع حقيقي، لا تختلقه)
...
- [ ] دمجت المواضيع الصغيرة المترابطة بدل تفتيتها لأقسام منفصلة (`combine_related_topics: true`)
- [ ] لا يوجد قسمان يشرحان نفس المفهوم من الصفر — أي تكرار مقصود مربوط بـ `⬅️ الربط مع السابق` ويذكر فقط الإضافة الجديدة
- [ ] `💡 الفكرة الأساسية` و`📖 الشرح` لا يعيدان نفس الجملة بصياغتين
- [ ] `⚠️ أخطاء شائعة` و`📚 التطبيق` موجودان فقط حيث فيه محتوى حقيقي لهما، لا حشواً لاستيفاء الهيكل — والأقسام شديدة البساطة استخدمت القالب المصغّر (Compact)
