(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      #ai-chat-box {
        ...
        flex-direction: column; font-family: ${font};
        resize: both;
      }
      ...
      .user-msg {
        ...
        background: ${primary}; color: ${onPrimary};
        align-self: flex-end; font-size: 13px; line-height: 1.5;
        border-bottom-right-radius: 4px;
      }
      .ai-msg {
        max-width: 85%; padding: 8px 12px; border-radius: ${radius};
        background: ${border}; color: ${onSurface};
        align-self: flex-start; font-size: 13px; line-height: 1.5;
      ...
      #ai-chat-send {
        ...
        cursor: pointer; font-size: 13px; font-family: ${font};
        transition: opacity 0.2s;
      }
  ...
  async function askQuestion(messages, input) {
    const question = input.value.trim()
    if (!question) return
    input.value = ''
    addMsg(messages, question, 'user')
    const thinking = addMsg(messages, 'Thinking...', 'ai')
  ...
  function init() {
    if (!document.querySelector('.section-block')) return
    injectStyles()
    buildHTML()
