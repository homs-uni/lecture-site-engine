...
import {
  initDiagrams,
  refreshDiagrams,
} from "../engine/renderer/diagram/diagram.js";
...
function resetHtmlCacheIfStale(buildId) {
  if (!buildId) return;
  if (htmlCacheBuildId && htmlCacheBuildId !== buildId)
    lectureHtmlCache.clear();
...
async function loadReviewManifest() {
  const res = await fetch(versionedUrl("reviews/manifest.json"), {
    cache: "no-store",
  });
...
async function loadReviewJson(path) {
  const res = await fetch(versionedUrl(`reviews/${path}`), {
    cache: "no-store",
  });
...
function getReviewIndexFromHash(hash) {
  ...
  if (idx >= 0) return idx;
  return appState.reviewItems.findIndex((it) =>
    hash.startsWith(`${it.review.id}-`),
  );
}

async function loadExamManifest() {
  const res = await fetch(versionedUrl("DAWRAT/manifest.json"), {
    cache: "no-store",
  });
  if (!res.ok) return null;
  return res.json();
...
async function loadExamJson(path) {
  const res = await fetch(versionedUrl(`DAWRAT/${path}`), {
    cache: "no-store",
  });
  if (!res.ok) throw new Error(`تعذّر تحميل ${path}`);
  return res.json();
...
function getExamIndexFromHash(hash) {
  ...
  if (idx >= 0) return idx;
  return appState.examItems.findIndex((it) =>
    hash.startsWith(`${it.exam.id}-`),
  );
}
...
function renderReviewFeatured() {
  ...
  document
    .getElementById("reviewFeaturedBtn")
...
function examStats(exam) {
  ...
  const count =
    mcqPart?.questions?.reduce(
      (n, q) => n + (q.type === "group" ? q.questions.length : 1),
      0,
    ) || 0;
  return { count };
}
...
function restoreDawratAnswers(root = document.getElementById("content")) {
  ...
  root
    .querySelectorAll('.section-block[data-part-type="mcq"]')
    .forEach((section) => {
...
function setJumpQuizVisible(show) {
  ...
  document
    ...
    ?.classList.toggle("hidden", !show);
  document
    .getElementById("mobileStudyQuizBtn")
    ?.classList.toggle("hidden", !show);
}
...
function loadReviewView(index, anchorHash) {
  ...
  const needsRender =
    currentReviewIndex !== index || !document.getElementById(item.review.id);
  ...
  if (needsRender) {
    ...
    document.getElementById("sidebarCourseTitle").textContent =
      shortLectureTitle(item.review.title);
    document.getElementById("sidebarCourseSub").textContent =
      item.review.tag || "";
    document.getElementById("sidebarMatIcon").textContent =
      item.matIcon || "menu_book";
    document.getElementById("mobileTocCourseTitle").textContent =
      shortLectureTitle(item.review.title);
    document.getElementById("mobileTocCourseSub").textContent =
      item.review.tag || "";
  ...
  const hash =
    anchorHash && anchorHash !== item.review.id ? anchorHash : item.review.id;
  routeLock = true;
...
function loadExamView(index, anchorHash) {
  ...
  const needsRender =
    currentExamIndex !== index || !document.getElementById(item.exam.id);
  ...
  if (needsRender) {
    ...
    document.getElementById("sidebarCourseTitle").textContent =
      shortLectureTitle(item.exam.title);
    document.getElementById("sidebarCourseSub").textContent =
      item.exam.tag || "";
    document.getElementById("sidebarMatIcon").textContent =
  ...
  const hash =
    anchorHash && anchorHash !== item.exam.id ? anchorHash : item.exam.id;
  routeLock = true;
  if (location.hash !== `#${hash}`) location.hash = hash;
  routeLock = false;
...
async function loadNotesManifest() {
  const res = await fetch(versionedUrl("notes/lectures/manifest.json"), {
    cache: "no-store",
  });
  if (!res.ok) return null;
  return res.json();
...
async function loadNoteJson(path) {
  const res = await fetch(versionedUrl(`notes/lectures/${path}`), {
    cache: "no-store",
  });
  if (!res.ok) throw new Error(`تعذّر تحميل ${path}`);
  return res.json();
...
function getNoteIndexFromHash(hash) {
  ...
  if (idx >= 0) return idx;
  return appState.notesItems.findIndex((it) =>
    hash.startsWith(`${it.lec.id}-`),
  );
}
...
async function loadNoteView(index, anchorHash) {
  ...
  const needsRender =
    currentNoteIndex !== index || !document.getElementById(item.lec.id);

  if (needsRender) {
    currentNoteIndex = index;
    setRefContext({
      lectureRef: item.lec.id,
      sectionMap: item.sectionIndex || {},
    });
    ...
    document.getElementById("sidebarCourseTitle").textContent =
      shortLectureTitle(item.lec.title);
    document.getElementById("sidebarCourseSub").textContent =
      item.lec.tag || "";
    document.getElementById("sidebarMatIcon").textContent =
      item.matIcon || "menu_book";
  ...
  const hash =
    anchorHash && anchorHash !== item.lec.id ? anchorHash : item.lec.id;
  routeLock = true;
  if (location.hash !== `#${hash}`) location.hash = hash;
  routeLock = false;
...
function lectureStats(lec) {
  ...
  return {
    ...
    mcq: mcqCount,
    sections:
      lec.parts?.find((p) => p.type === "detail")?.subsections?.length || 0,
  };
}
...
function renderSubjectProgressTracker() {
  ...
  const progress =
    appState.progressTracker.getSubjectProgress(subjectLectureIds());
...
function initLectureCompletionButtons() {
  ["sidebarCompleteBtn", "mobileCompleteBtn"].forEach((id) => {
    ...
    btn.addEventListener("click", () => {
      ...
      toggleLectureCompletedByIndex(
        idx,
        id === "sidebarCompleteBtn" ? "sidebar" : "mobile",
      );
    });
  });
}
...
async function loadManifest() {
  const res = await fetch(versionedUrl("lectures/manifest.json"), {
    cache: "no-store",
  });
  ...
  const visibleItems = appState.items
    ...
    .filter(
      ({ item }) =>
        ...
        item.fileMeta?.section === appState.activeSection,
    );
      ...
      <article class="lecture-picker-card group text-right flex flex-col justify-between items-start bg-surface-container-lowest border border-outline-variant rounded-2xl p-lg custom-shadow box-hover w-full"
        ...
        <div class="flex items-center justify-between mb-md w-full">
          ...
          <button type="button" class="lecture-complete-btn flex items-center gap-xs text-on-surface-variant hover:text-primary transition-colors text-sm font-label-md shrink-0"
            ...
            ${ms(isDone ? "check_circle" : "radio_button_unchecked", isDone, `text-base ${isDone ? "text-green-600" : "text-on-surface-variant"}`)}
            <span>${doneLabel}</span>
          </button>
        </div>
       ...
       <!-- Action Button (Bottom) -->
        <div class="w-full mt-auto">
          <button type="button" class="lecture-open-btn group w-full py-md bg-primary text-on-primary hover:opacity-90 active:scale-[0.98] rounded-xl font-label-md font-bold inline-flex items-center justify-center gap-sm transition-all shadow-sm" data-open-lecture-index="${i}" aria-label="فتح ${escAttr(title)}">
            <span>ابدأ الدراسة</span>
    ${ms("arrow_back", false, "text-lg group-hover:-translate-x-1 transition-transform")}
         </button>
        </div>
      </article>`;
    })
...
function revealAnimated(el) {
  if (!el) return;
  const section = el.classList.contains("section-block")
    ? el
    : el.closest(".section-block");
  const targets = section
    ? [section, ...section.querySelectorAll(".box-animate")]
    : [el];
...
function scrollToAnchor(anchorHash, attempt = 0) {
  ...
  const far =
    Math.abs(el.getBoundingClientRect().top) > window.innerHeight * 1.5;
...
function revealLectureDetailSections(
  root = document.getElementById("content"),
) {
  if (!root) return;
  root
    .querySelectorAll('.section-block[data-part-type="detail"]')
    .forEach(revealSectionTree);
}
...
function buildSidebar(toc) {
  ...
  toc.parts.forEach((part) => {
    ...
    containers.forEach((container) => {
      ...
      (part.subsections || []).forEach((sub) => {
        ...
        const indent =
          sub.level >= 5 ? "mr-2xl" : sub.level >= 4 ? "mr-xl" : "mr-lg";
        subLink.href = `#${subId}`;
  ...
  document.querySelectorAll(".toc-nav-link").forEach((link) => {
    ...
    const primary = allLinks.find(
      (l) => l.el.getAttribute("href") === link.getAttribute("href"),
    );
    ...
    link.addEventListener("click", (e) => {
      ...
      if (!anchorId) return;
      const partType =
        link.dataset.partType || target?.getAttribute("data-part-type") || "";
      trackTocNavigated({
        targetId: anchorId,
        partType,
        isSubsection:
          link.classList.contains("toc-nav-link--sub") ||
...
function updateSidebarProgressFill() {
  ...
  const frac = Math.min(
    1,
    Math.max(0, (window.scrollY + SCROLL_OFFSET_PX - contentTop) / span),
  );
...
function initSidebarLectureNav() {
  [
    ...
    .filter(Boolean)
    .forEach((btn) =>
      btn.addEventListener("click", () => goToAdjacentLecture(-1)),
    );
  [
    document.getElementById("sidebarNextLectureBtn"),
    document.getElementById("mobileNextLectureBtn"),
...
function initScrollAnimations(root = document) {
  ...
  root
    .querySelectorAll(".lecture-body .section-block")
    .forEach(revealSectionTree);
}
...
async function loadLectureView(idx, hashPart) {
  ...
  const needsRender =
    currentLectureIndex !== idx || !document.getElementById(item.lec.id);
  currentLectureIndex = idx;
  setJumpQuizVisible(!!item.lec.parts?.find((p) => p.type === "mcq"));
...
function jumpToSummary() {
  ...
  const isQuickSummary = (title) =>
    /سريع|قبل البدء|قبل المحاضرة|اقرأ قبل|checklist|قائمة فحص|قائمة المراجعة/i.test(
      title || "",
    );
  const isComprehensive = (title) =>
    /شامل|Alternative Complete|قراءة بديلة/i.test(title || "");
  ...
  if (!targetId) {
    const nodes = [
      ...document.querySelectorAll(
        '#content .section-block[data-part-type="summary"]',
      ),
    ];
    const byTitle = (pred) =>
      nodes.find((el) => pred(el.querySelector("h3")?.textContent || ""));
    targetId =
...
function initJumpSummary() {
  document
    .getElementById("jumpQuizBtn")
    ?.addEventListener("click", jumpToSummary);
  document
    .getElementById("mobileJumpQuizBtn")
    ?.addEventListener("click", jumpToSummary);
  document
...
function bindJumpSummaryClicks() {
  document
    ...
    ?.addEventListener("click", onJumpSummaryClick);
}
...
function bindExpandOriginalInlineToggle(
  root = document.getElementById("content"),
) {
  if (!root) return;
  root.querySelectorAll("[data-expand-original-checkbox]").forEach((input) => {
...
function openMobileToc() {
  ...
  document
    .getElementById("mobileTocDrawer")
    ?.setAttribute("aria-hidden", "false");
  document
    .getElementById("mobileTocBackdrop")
    ?.setAttribute("aria-hidden", "false");
...
function initMobileStudyUi() {
  document
    .getElementById("mobileTocOpen")
    ?.addEventListener("click", openMobileToc);
  document
    .getElementById("mobileTocClose")
    ?.addEventListener("click", closeMobileToc);
  document
    .getElementById("mobileTocBackdrop")
    ?.addEventListener("click", closeMobileToc);
  document
    .getElementById("mobileScrollTopBtn")
    ?.addEventListener("click", () => {
      window.scrollTo({ top: 0, behavior: "smooth" });
    });
}
...
async function init() {
  ...
  document
    ...
    ?.addEventListener("click", goToSubjectHome);
  document
    .getElementById("backToHubBtn")
    ?.addEventListener("click", goToHubHome);
  document
    .getElementById("brandBtn")
    ?.addEventListener("click", handleBrandClick);
  ...
  try {
    ...
    siteTitle =
      manifest.settings?.subjectName ||
      manifest.title ||
      GUIDE_CONFIG.defaultTitle;
    ...
    appState.progressTracker = createProgressTracker({
