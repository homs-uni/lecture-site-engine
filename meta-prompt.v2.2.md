# Meta-Prompt v2.2 — Generate Subject-Specific Lecture Extraction Prompt

> **What changed in v2.2 (vs v2.1):** `content_ordering.default_type` grows from
> four lanes to seven. Everything else is identical to v2.1.
>
> 1. **`comparison-first`** (new, enhanced) — for subjects organized around
>    "here are N options, pick by criteria" (protocol choices, architecture
>    trade-offs, model selection). The comparison matrix leads the section,
>    not a supporting table buried after the prose.
> 2. **`command-first`** (new) — for CLI/tool/config-driven material where the
>    artifact is a command or config block, not a program (`code-first`) or a
>    picture (`diagram-first`).
> 3. **`scenario-first`** (new) — for subjects that open with a real incident
>    or case before extracting the abstract concept (security, ethics-adjacent
>    material).
> 4. **`process-first` was considered and rejected as a standalone lane.**
>    Lifecycle/methodology subjects (SDLC, project stages, CRISP-DM) still use
>    `diagram-first` — draw the stage sequence as a `graph LR/TD` chain instead
>    of inventing a fifth "no-picture" ordering. See the note under
>    `diagram-first` below.
>
> **Why it exists:** subjects like Information Systems were being forced into
> `diagram-first` globally just because they contain ER/DFD diagrams, when the
> actual spine of the material (SDLC stages, model-selection trade-offs, tool
> commands) needed a different section shape. `content_ordering` should match
> what's actually teaching the student, not the domain label.
>
...
- Respect `content_ordering.default_type` from subject-brief — one of `equation-first | code-first | diagram-first | comparison-first | command-first | scenario-first | prose-first`. Each puts a different element first in every section (formula / code / mermaid diagram / comparison matrix / command block / real scenario / prose respectively) — see "ترتيب المحتوى حسب نوع المادة" below. Don't default to `diagram-first` just because the subject has some diagrams — check what element actually leads most sections.
...
> 📌 **مواد المراحل ودورة الحياة (Process / Lifecycle) تستخدم `diagram-first` أيضاً —
> ولا تحتاج نوعاً منفصلاً.** ارسم تسلسل المراحل كمخطط `graph LR` أو `graph TD` واحد
> متصل، ثم اشرح كل مرحلة (📥 المدخلات، 🔧 الأنشطة، 📤 المخرجات) كقسم فرعي بعده.
>
> **مثال — دورة حياة تطوير الأنظمة (SDLC):**
> ```mermaid
> graph LR
>     P["Planning<br/>وثيقة المشروع"] --> A["Analysis<br/>جمع المتطلبات"]
>     A --> D["Design<br/>ERD / DFD"]
>     D --> I["Implementation<br/>الكود + الاختبار"]
>     I --> M["Maintenance<br/>تصحيح / تحسين"]
> ```
> ثم لكل مرحلة قسم `### 4.1 Planning` بصيغة: 💡 الفكرة الأساسية → 📥 المدخلات →
> 🔧 الأنشطة → 📤 المخرجات (Deliverable) → ⬅️ كيف تقود للمرحلة التالية.
> لا تخترع نوع محتوى خامس لهذا — المخطط + بنية المراحل كافية.

---

### للمقارنات والقرارات متعددة المعايير:
**نوع المحتوى:** `type: "comparison-first"`

**متى تستخدمه:** الموضوع أصله "عندك N خيارات، تختار حسب معايير" — وليس تعريفاً
واحداً ولا مخططاً واحداً. أمثلة: اختيار نموذج `SDLC`، بروتوكولات الشبكة،
سياسات استبدال الصفحات (`page replacement`)، خوارزمية الفرز المناسبة لحجم البيانات.

**لماذا احتاج نوعاً منفصلاً عن `diagram-first`؟** لأن قسراً وضع جدول مقارنة
تحت "📊 المخطط" يخلط بين أداتين مختلفتين — الجدول **هو** المحتوى الأساسي هنا،
وليس شرحاً مرافقاً لمخطط.

**الترتيب الإلزامي لكل قسم (`### 1.1`):**
1. العنوان + metadata
2. 📍 أين نحن الآن؟
3. ⬅️ الربط مع السابق
4. 💡 الفكرة الأساسية (جملة واحدة تلخّص "بين ماذا نقارن ولماذا")
5. **📋 جدول المقارنة (`Multi-criteria matrix`)** ← **يأتي أولاً قبل أي شرح**
6. **🎯 إطار القرار (`Decision framework`)** — أسئلة توجيهية تقود لاختيار الصف الصحيح
7. 📖 الشرح — لماذا يختلف كل خيار (سبب واحد جوهري لكل صف، ليس إعادة سرد الجدول)
8. ⚖️ المقايضة (إن لم تُغطَّ بالكامل في الجدول)
9. 🎯 الملخص السريع
10. 📚 التطبيق
11. ⚠️ أخطاء شائعة (الخلط الشائع بين خيارين متشابهين)
12. 📄 النص الأصلي (collapsible)

**بنية جدول المقارنة — ثلاثة أعمدة إلزامية كحد أدنى:**

```markdown
#### 📋 جدول المقارنة: [اسم القرار]

| المعيار | الخيار A | الخيار B | الخيار C |
| --- | --- | --- | --- |
| [معيار 1 — مثال: وضوح المتطلبات] | [قيمة/تقييم] | [قيمة/تقييم] | [قيمة/تقييم] |
| [معيار 2 — مثال: حجم المشروع] | ... | ... | ... |
| [معيار 3 — مثال: المرونة للتغيير] | ... | ... | ... |
| **الأنسب لـ** | [سيناريو محدد] | [سيناريو محدد] | [سيناريو محدد] |
```

**إطار القرار — صيغة شجرة أسئلة أو جدول Quick Reference:**

```markdown
#### 🎯 إطار القرار: كيف تختار؟

| إذا كان السؤال... | والجواب... | اختر |
| --- | --- | --- |
| المتطلبات واضحة تماماً؟ | نعم | Waterfall |
| المتطلبات واضحة تماماً؟ | لا، متغيرة | Iterative / Agile |
| المشروع عالي المخاطر؟ | نعم | Spiral |
```
...
| --- | --- | --- | --- |
| وضوح المتطلبات | مطلوب مسبقاً | يمكن تطويره تدريجياً | غامض ومتغير |
| المخاطر | منخفضة | متوسطة | عالية — يُدار بشكل صريح |
| **الأنسب لـ** | نظام بنكي بمواصفات ثابتة | تطبيق ويب متوسط الحجم | مشروع بحثي كبير غير مؤكد |

#### 🎯 إطار القرار
لو المتطلبات ثابتة ولا تتوقع تغييرها → Waterfall.
لو تتوقع تغييرات لكن المخاطر مقبولة → Iterative.
لو المخاطر عالية وتحتاج تقييمها في كل مرحلة → Spiral.

#### 📖 الشرح
الفرق الجوهري مو في "الخطوات" (كلها تمر بنفس المراحل تقريباً) بل في **متى تُقفل كل مرحلة**...
```

**✅ قائمة تحقق `comparison-first`:**
- [ ] جدول المقارنة **يسبق** أي فقرة شرح — هو نقطة الدخول للقسم
- [ ] الجدول فيه **صف "الأنسب لـ"** بسيناريو محدد لكل عمود (وليس صفة عامة)
- [ ] إطار القرار موجود (سؤال → قرار)، وليس فقط جدولاً وصفياً
- [ ] الشرح النثري يفسّر **سبب** الفرق، لا يعيد سرد خلايا الجدول
- [ ] كل صفوف الجدول لها نفس عدد الخلايا

---

### للأوامر والإعدادات:
**نوع المحتوى:** `type: "command-first"`

**متى تستخدمه:** الموضوع أساسه أمر CLI، ملف إعداد (`config`)، أو استدعاء أداة —
وليس برنامجاً كاملاً (`code-first`). أمثلة: أوامر إعداد جهاز شبكة، أوامر
مراقبة (`SNMP`)، أوامر `adb`/`Gradle`، استعلامات إدارة قاعدة بيانات.
...
3. ⬅️ الربط مع السابق
4. 💡 الفكرة الأساسية
5. **💻 الأمر / الإعداد** ← **يأتي أولاً**
6. شرح كل جزء/خيار (`flag`) في الأمر — قائمة مرقّمة
7. 📖 الشرح: "ماذا يحدث فعلياً عند تنفيذه؟"
8. 📤 المخرج المتوقع (`expected output`)
9. 🛠️ استكشاف الأخطاء (أخطاء شائعة عند التنفيذ)
10. 🎯 الملخص السريع
11. 📚 التطبيق
12. 📄 النص الأصلي (collapsible)
...
**شرح الخيارات:**
- **`interface`**: يحدد الكائن المطلوب عرضه
- **`status`**: يطلب الحالة الحالية فقط (وليس التفاصيل الكاملة)
...
### للسيناريوهات والحالات الواقعية:
**نوع المحتوى:** `type: "scenario-first"`

**متى تستخدمه:** الموضوع يبدأ منطقياً من حادثة/حالة واقعية قبل استخلاص المفهوم
المجرد — أمثلة: أمن المعلومات (هجوم → الثغرة التي استغلها → المبدأ العام)،
دراسات حالة في هندسة الأنظمة.

**الترتيب الإلزامي:**
1. العنوان + metadata
2. **📌 السيناريو** ← **يأتي أولاً، قبل أي تعريف**
3. ❓ ما المشكلة هنا بالضبط؟
4. 💡 المفهوم الذي يفسّرها
5. 📖 الشرح العام (التعميم — القاعدة التي تنطبق خارج هذا المثال)
6. ⬅️ الربط مع السابق
7. 🎯 الملخص السريع
8. 📚 التطبيق (سيناريوهات أخرى يظهر فيها نفس المفهوم)
9. ⚠️ أخطاء شائعة
10. 📄 النص الأصلي (collapsible)

```markdown
#### 📌 السيناريو
موظف يفتح مرفقاً في بريد إلكتروني يبدو من قسم الموارد البشرية، فيتم تشفير كل
ملفات جهازه خلال دقائق.

#### ❓ ما المشكلة هنا بالضبط؟
الموظف لم يتحقق من مصدر البريد، والمرفق كان ينفّذ كوداً بمجرد فتحه.

#### 💡 المفهوم الذي يفسّرها
**هذا مثال على `Ransomware` عبر `Phishing` — استغلال ثقة المستخدم بدل ثغرة تقنية.**
```
...
- [ ] `content_ordering.default_type` مُعكوس في التعليمات — واختير بناءً على العنصر الذي يقود أغلب الأقسام فعلياً (وليس افتراض `diagram-first` لمجرد وجود بعض المخططات)
- [ ] إن كان النوع `comparison-first`: كل قسم يبدأ بجدول مقارنة + إطار قرار قبل أي فقرة شرح
- [ ] إن كانت المادة تحتوي مراحل/دورة حياة (Process/Lifecycle): استُخدم `diagram-first` مع مخطط تسلسل مراحل واحد — لم يُخترع نوع خامس
